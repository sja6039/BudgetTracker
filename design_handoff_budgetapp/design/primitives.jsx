...
// Minimalist 1.5px stroke icon set
...
// AreaLine — single-series area + line, style 1 (clean, gradient fill)
const AreaLine = ({ data, height = 200, accent = 'var(--accent)', style = 'area', showAxis = true, padX = 0, padY = 14 }) => {
  const w = 100, h = 100;
  const min = Math.min(...data), max = Math.max(...data);
  const range = max - min || 1;
  const pts = data.map((v, i) => [
    (i / (data.length - 1)) * (w - padX * 2) + padX,
  ...
  if (style === 'bar') {
    ...
    return (
      <svg viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" style={{ width: '100%', height, display: 'block' }}>
        {data.map((v, i) => {
          const x = (i / (data.length - 1)) * (w - padX * 2) + padX - bw / 2;
...
const Spark = ({ data, color = 'var(--accent)', height = 36, width = 110 }) => {
  const min = Math.min(...data), max = Math.max(...data);
  const range = max - min || 1;
  const pts = data.map((v, i) => [
...
const Donut = ({ slices, size = 180, thickness = 22 }) => {
...
Object.assign(window, { Ico, AreaLine, Spark, Donut, money });
